...
const collectionname = "slatesids"
...
// uncoment this function when you remove functions for POST methods of epicbox
// POST method is not used in other epicboxes and in wallets in current time.
/*
...
//
// all functions from here to server definition are used only for POST methods,
// you can remove them and uncomment above funticon if you don't need POST method,
// Cod will be more clera for you then.
//
      ...
      try {
        ...
        req.on('end', function() {
          console.log('Body: ' + body)
           ...
           switch (json.type) {
              case 'Challenge':
                console.log('Post ask Challenge');
                forpostChallenge(res, json)
                break;
              case 'Subscribe': 
                console.log('Subscribe');
                forpostSubscribe(res, json)
                break;
              case 'PostSlate':
                console.log('Post ask PostSlate');
                forpostPostSlate(res, json)
                break;
              

              case 'Made':
                console.log('Post ask Made');
                forpostMade(res, json)
                break;
              default:  
                
                
                res.end(JSON.stringify({type:"Error", kind:"bad request", description:"I don't use this function"}))
            }
       ...
       res.end(JSON.stringify({type:"Error", kind:"Wrong request", description:"Can't parse json"}));
      }
...
function forpostChallenge(res, json){
    
    res,end(JSON.stringify({type:"Challenge", str:challenge}))
 ...
 try{
  const child = execFile('./epicboxlib', ["verifysignature", json.address , challenge, json.signature], (error, stdout, stderr) => {
     ...
     if(isTrueSet) {
              ...
              try {
                    ...
                    if(json.address!=null) ch2.get(json.address, {noAck:false}, (err, msgrabbit)=>{
                        if(msgrabbit!=false && err==null) {
                          ...
                          //answer.id = msgrabbit.properties;
                          console.log(msgrabbit.fields)
                          // msgrabbit.properties.messageId usefull empty
                          ...
                          if(json.hasOwnProperty("ver") && json.ver=="2.0.0" && msgrabbit.properties.hasOwnProperty("messageId")){
                            ...
                            collection.insertOne({ messageid: messageid, slate:msgrabbit }).then((insertResult)=>{
                              ...
                              res.end(answerstr)
                                ...
                                //let binar = Buffer.from(answerstr, 'latin1');
                         ...
                         //console.log("Error ", err)
                          ...
                          res.end(JSON.stringify({type:"Error", kind:"No more slates for you", description:"No more slates here now, try subscribe later"}))
                        }

                                
                        ch2.close((err2)=>{
                           if(err2!=null)  console.error(err2)
                        })       
                        ...
                        res.end(JSON.stringify({type:"Error", kind:"No more slates for you", description:"Ineternal rabbit erro - slates here now, try subscribe later"}))
      ...
      res.end(JSON.stringify({type:"Error", kind:"signature error", description:"Signature error"}))
     ...
     const child = execFile('./epicboxlib', ["verifysignature", json.address , challenge, json.signature], (error, stdout, stderr) => {
         ...
         if(isTrueSet) {
            ...
            rabbitconn.createChannel((err1, ch2) => {
              ...
              if(json.hasOwnProperty("ver") && json.ver=="2.0.0" && json.hasOwnProperty("epicboxmsgid")){
                   
                      const db = mongoclient.db(dbName);
                      const collection = db.collection(collectionname);

                      collection.findOne({ messageid: json.epicboxmsgid, "slate.fields.routingKey": json.address}).then((findResult)=>{

                        console.log('Find document =>', findResult);
                        ...
                        if(findResult) { 
                          ...
                          //console.log(findResult.slate.fields) 
                          //ch2.ack(findResult.slate)
      ...
      res.end(JSON.stringify({type:"Error", kind:"made error", description:"Made error"}))
...
function forpostPostSlate(res, json){

  try {
       console.log("post_postslate from ", json.from, "to ", json.to)
     ...
     const childadd = execFile('./epicboxlib', ['verifyaddress',  json.from, json.to], (erroradr, stdoutadr, stderradr) => {
        ...
        if(isTrueSetadr) {
                     ...
                     const child2 = execFile('./epicboxlib', ["verifysignature", from , challenge, json.signature], (error, stdout, stderr) => {
                         ...
                         if(isTrueSet2){
                           preparePostSlate(ws, json, challenge);
                             ...
                             res.end(JSON.stringify({type:"Error", kind:"Post Slate Signature error", description:"Post Slate Signature error"}))
          ...
          res.end(JSON.stringify({type:"Error", kind:"Post Slate Address error", description:"Post Slate Address error"}))
    ...
    console.error("postslate ", err)
     res.end(JSON.stringify({type:"Error", kind:"Post Slate error", description:"Post Slate error"}))
...
//
// function above are for POST methods
// more of you never will need it
// so you can remove it for less code 
//
//
